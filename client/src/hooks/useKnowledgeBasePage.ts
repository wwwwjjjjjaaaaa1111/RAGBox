...
export function useKnowledgeBasePage() {
  ...
  async function refreshRows(options?: { silent?: boolean }) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "加载知识库文件失败";
  ...
  async function handleFilesSelected(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      setUploadMessage(`正在准备上传：${file.name}`);
      ...
      try {
        const result = await uploadFileToKnowledgeBase(file, {
          onUploadingProgress: (percent) => {
            ...
            setUploadMessage(`上传中 ${value}%：${file.name}`);
          },
          onPhaseChange: (phase) => {
            setUploadPhase(phase);
            if (phase === "hashing") {
              setUploadProgress(0);
              setUploadMessage(`正在计算哈希：${file.name}`);
            }
            if (phase === "uploading") {
  ...
  async function handleDispatchRow(row: UploadLibraryRow) {
    ...
    try {
      ...
      setUploadMessage(`已发送入库请求：${row.name}`);
      await refreshRows({ silent: true });
    } catch (error) {
      updateQueuedDispatchRowIds(queuedDispatchRowIdsRef.current.filter((id) => id !== row.id));
      setRows(previousRows);
      const message = error instanceof Error ? error.message : "发起入库失败";
      setLoadError(message);
    } finally {
