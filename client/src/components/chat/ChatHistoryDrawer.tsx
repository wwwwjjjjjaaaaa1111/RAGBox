...
const ChatHistoryDrawer = ({
  ...
  // 抽屉内部维护“Select模式”和勾选集合，这些状态只服务于当前 UI 展示。
    ...
    <aside className="hidden w-80 shrink-0 flex-col border-l border-slate-100 bg-white xl:flex">
      <div className="border-b border-slate-50 p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="font-headline mb-1 text-lg font-bold tracking-tight text-black">最近会话</h2>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400">聊天历史</p>
          </div>
          <button
        ...
        {isSelectionMode ? (
          <div className="mt-4 flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-4 py-3">
            <span className="text-xs font-medium text-slate-500">已选 {selectedSessionIds.length} 项</span>
            <button
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <span className="line-clamp-2">{item.title || "新对话"}</span>
                    </button>
                    <button
