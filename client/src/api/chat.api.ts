...
import { AUTH_EXPIRED_EVENT, createApiUrl, requestApi } from "./httpClient";
...
export async function createChatSession(userId: string, title?: string, fileIds?: string[]) {
  return requestApi<ChatSession>("/chat/sessions", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ userId, title, fileIds }),
  });
}
 ...
 */
export async function updateSessionFiles(sessionId: string, fileIds: string[]) {
  return requestApi<{ id: string; fileIds: string[] }>(`/chat/sessions/${sessionId}/files`, {
    method: "PUT",
...
export async function streamChatCompletion(
  ...
  if (!response.ok || !response.body) {
    if (response.status === 401) {
      // Keep session-expiry handling consistent with requestApi.
      clearAuthSession();
