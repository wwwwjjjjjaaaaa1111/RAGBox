...
function parseFileIds(value: string | null | undefined): string[] {
  if (!value) {
    return [];
  }
...
function parseSessionFields(fileIdsJson: string | null | undefined) {
  return {
    fileIds: parseFileIds(fileIdsJson),
    fileIdsJson: undefined as string | null | undefined,
  };
}
 ...
 * @returns 新创建的会话记录。
 */
export async function createChatSession(payload: { userId: string; title?: string; fileIds?: string[] }) {
  const session = await chatRepository.createSession(payload.userId, payload.title || null, payload.fileIds || []);
  return {
    ...session,
    ...parseSessionFields(session.fileIdsJson),
    messageCount: 0,
  };
}
...
/**
 * 解析会话上持久化的检索范围文件 ID 列表（供 completions 转发使用）。
 * @param value 会话记录的 fileIdsJson 字段。
 * @returns 文件 ID 数组（无限制时为空数组）。
 */
export function parseSessionFileIds(value: string | null | undefined): string[] {
  return parseFileIds(value);
}

/**
 * 更新会话标题（自动生成流程使用）。
 * @param sessionId 会话 ID。
 * @param userId 用户 ID（用于属主校验）。
 * @param title 新标题。
 */
...
/**
 * 更新会话限定的检索文件范围。
 * @param payload 会话 ID、用户 ID 与文件 ID 列表。
 * @returns 会话 ID 与生效的 fileIds。
 * @throws SESSION_NOT_FOUND 当会话不存在或不属于该用户时抛出。
 */
export async function setSessionFiles(payload: { sessionId: string; userId: string; fileIds: string[] }) {
  await requireOwnedSession(payload);
  const updated = await chatRepository.updateSessionFileIds(payload.sessionId, payload.fileIds);
  return {
    id: updated.id,
    fileIds: parseFileIds(updated.fileIdsJson),
  };
}
