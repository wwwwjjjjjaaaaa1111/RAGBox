 ...
 * @param fileIds 限定的知识库文件 ID 列表（空/缺省 = 检索全部文件）。
 * @returns 新创建的会话记录。
 */
export async function createSession(userId: string, title: string | null, fileIds: string[] = []) {
  return prisma.chatSession.create({
    data: {
      userId,
      title,
      fileIdsJson: fileIds.length ? JSON.stringify(fileIds) : null,
    },
  });
}
 ...
 * 更新会话限定的检索文件范围。
 * @param id 会话 ID。
 * @param fileIds 文件 ID 列表；空数组存 null（= 检索全部）。
 * @returns 更新后的会话记录。
 */
export async function updateSessionFileIds(id: string, fileIds: string[]) {
  return prisma.chatSession.update({
    where: { id },
    data: { fileIdsJson: fileIds.length ? JSON.stringify(fileIds) : null },
  });
}
