...
src/
  ...
  services/
    ...
    auth.service.ts          # Account/session auth
    ingestionEvents.ts       # SSE fanout for ingestion updates
  routes/
    index.ts                 # Main route map
    auth.routes.ts           # register/login/logout/me
    ai.routes.ts             # AI-server callback endpoints
  middleware/
    auth.ts                  # Bearer/query-token session guard
    upload.ts                # Multer config
  repositories/
    user.repository.ts       # user record read helpers
...
- Accounts use username + password; passwords are hashed with scrypt + per-user salt.
- Login/register issue an opaque session token stored hashed (`sha256`) in `auth_sessions`.
- All business endpoints require `Authorization: Bearer <token>` (SSE endpoints accept `?token=`).
- `requireAuth` derives `userId` from the session and overwrites any client-supplied `userId`, so resource queries are always scoped to the authenticated account.
...
- `POST /v1/chat/sessions/:id/completions` (streaming SSE proxy)

Behavior:

- Sessions are scoped to the authenticated user.
- Session listing exposes `messageCount`.
- Message create/list validates session existence first; destructive ops verify ownership.
- `completions` persists the user message, loads recent history
  (`CHAT_CONTEXT_MESSAGE_LIMIT`, default 12), and streams events from Python
  `/chat/stream` back to the client while persisting the assistant message.

### 5.4 AI-Service Callbacks
...
- Both endpoints are called by the Python service and authenticated by `x-ai-service-secret`.
- Callback updates the task status/progress and broadcasts an SSE event.
- Chunk sync replaces the file chunk metadata after vectors are written.
...
- `AUTH_SESSION_TTL_HOURS` (default `72`)
...
- `AI_VECTOR_DELETE_ENDPOINT` (optional override; defaults to `{AI_SERVICE_BASE_URL}/vectors/files`)
...
- `CHAT_CONTEXT_MESSAGE_LIMIT` (default `12`, keep in sync with AI-server)
